...
class TestRecConcave(unittest.TestCase):
    ...
    def setUp(self):
        ...
        bulk_qualities = src.qualities.bulk_quality_minmax(self.data, range(self.range_end))
        ...
        plt.plot(range(self.range_end), src.qualities, 'bo')
        plt.show()

    def test_rec_concave_basis_median(self):
        print "testing basis to find median"

        result_depth_1 = src.rec_concave.evaluate(self.data, self.range_end, src.qualities.bulk_quality_minmax, self.maximum_quality,
                                    self.alpha, self.eps, self.delta, 1, True)
        print "result from rec_concave: %d" % result_depth_1
        result_quality = src.qualities.quality_minmax(self.data, result_depth_1)
    ...
    def test_rec_concave_median(self):
        ...
        result_depth_2 = src.rec_concave.evaluate(self.data, self.range_end, src.qualities.bulk_quality_minmax,
                                              self.maximum_quality, self.alpha, self.eps, self.delta, 2, True)
        print "result from rec_concave: %d" % result_depth_2
        result_quality = src.qualities.quality_minmax(self.data, result_depth_2)
        print "and its quality: %d \n" % result_quality
        self.assertLessEqual(np.abs(result_quality - self.maximum_quality), 10)
